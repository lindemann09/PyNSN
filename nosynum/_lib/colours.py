...
def convert_colour(value):
    ...
    if value is None:
        return None
    try:
        hextriplet2rgb(value)  # check if vaild hextriplet
        if value[0] != "#":
            value = "#" + value
        return value.upper()
    except:
    ...
    try:
        if len(value) == 3:
            return rgb2hextriplet(value)
    except:
        pass

    raise RuntimeError("Incorrect colour ('{}'). Use RGB tuple, hex triplet or colours.named_colour.".format(value))
