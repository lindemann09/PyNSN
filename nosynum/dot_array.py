...
class DotArray(object):
    ...
    def __init__(self, stimulus_area_radius, n_dots,
        ...
        self.dot_diameter_mean = dot_diameter_mean #TODO:maybe rename to make
    ...
    def create_dots(self):
        self._dots = []
        if self.dot_diameter_range is None or \
             self.dot_diameter_std is None:
            #constant mean
            self._dots = [Dot(diameter=self.dot_diameter_mean,
                                colour=self.dot_colour) \
                         for _ in range(self.n_dots)]
        else:
            # draw diameter from beta distribution
            parameter = shape_parameter_beta(self.dot_diameter_range,
                                self.dot_diameter_mean, self.dot_diameter_std)
    ...
    def get_array_csv_text(self, index=0, num_format="%10.2f",
    ...
    @property
    def dot_limitation(self):
        return self._dot_limitation

    @dot_limitation.setter
    def dot_limitation(self, value):
        """Dot limitations

        If dot_limitations (integer) is defined (!=None), only the first
    ...
    @property
    def properties(self):
        return [self.n_dots, self.mean_dot_diameter, self.total_area,
    ...
    def density2(self):
        ...
        return math.pi * self.stimulus_area_radius**2 / self.total_area
    ...
    def create_pil_image(self, area_colour=None, convex_hull_colour=None,
        ...
        pict_size = int(round(self.stimulus_area_radius * 2))
        def convert_pos(xy):
        ...
        if area_colour is not None:
            draw_dot(draw, Dot(x=0, y=0, diameter=self.stimulus_area_radius*2),
                    colour = area_colour)
        if convex_hull_colour is not None:
            #plot convey hull
            hull = self.convex_hull
