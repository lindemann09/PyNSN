...
class ArrayProperties(object):
    ...
    def average_rectangle_size(self) -> NDArray:
        if not isinstance(self._oa, RectangleArray) or self.numerosity == 0:
    ...
    def to_dict(self) -> dict:
        ...
        if isinstance(self._oa, DotArray):
            rtn[2] = (
                VisualPropertyFlags.AV_DOT_DIAMETER.label(),
    ...
    def as_text(self, with_hash: bool = False, extended_format: bool = False) -> str:
        if extended_format:
            rtn = None
    ...
    def fit_average_diameter(self, value: float) -> None:
        ...
        if not isinstance(self._oa, DotArray):
            raise TypeError(
                "Adapting diameter is not possible " + f"for {type(self._oa).__name__}."
            )
    ...
    def fit_average_rectangle_size(self, value: ArrayLike) -> None:
        ...
        if new_size.shape != (2,):
            raise ValueError(
                f"Value ({value}) has to tuple of 2 numerical " + "(width, height)."
            )

        av_size = self.average_rectangle_size
        if not np.all(av_size > 0):
            raise RuntimeError("Numerosity, width or hight is zero or not defined.")
