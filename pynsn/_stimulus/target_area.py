...
class TargetArea(object):
    def __init__(
        self, shape: Dot | Rectangle | Ellipse | PolygonShape, min_dist_boarder: int = 2
    ) -> None:
        super().__init__()
        ...
        if shape.height < 1 or shape.width < 1:
            raise RuntimeError(f"Target area is too small. size={shape.size}")

        self._shape = shape
        if np.any(shape.xy != 0):
            warnings.warn(
                "TargetArea does not use shape position. "
        ...
        f = np.array((-0.5, 0.5))
        self._default_distr = Uniform2D(
            x_minmax=shape.size[0] * f, y_minmax=shape.size[1] * f
        )
    ...
    def random_position(
        self, shape: AbstractShape, max_iterations: int | None = None
    ) -> AbstractShape:
        ...
        while True:
            if cnt > max_iterations:
                raise NoSolutionError("Can't find a free position for this polygon")
            cnt += 1
