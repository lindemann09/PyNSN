...
class NSNStimulusPair:
    def __init__(
        self, stim_a: NSNStimulus, stim_b: NSNStimulus, name: str = "no_name"
    ) -> None:
        self.stim_a = stim_a
        self.stim_b = stim_b
        self.name = name
    ...
    def to_json(
        ...
        a = self.stim_a.to_json(indent=indent, tabular=tabular, decimals=decimals)
        b = self.stim_b.to_json(indent=indent, tabular=tabular, decimals=decimals)
        c = '{"name":"' + self.name + '"}'
        json_str = f"[{c},\n{a},\n{b}]"
    ...
    def from_json(path: str | Path) -> NSNStimulusPair:
        ...
        return NSNStimulusPair(
            stim_a=NSNStimulus.from_dict(a),
    ...
    def property_difference(self, prop: str | VP) -> np.float64:
        ...
        prop = ensure_vp(prop)
        rtn = self.stim_a.properties.get(prop) - self.stim_b.properties.get(prop)
