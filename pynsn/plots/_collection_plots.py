...
from ..collections._coll_stim_pairs import CollectionStimuli, CollectionStimulusPairs


def scatter_matrix(
    stimuli: CollectionStimuli,
...
def property_regression(
    ...
    return _regression_plots(
        data[cols], dv=dv, ivs=iv_prop, figsize=figsize, alpha=alpha
    )


def property_difference_regression(
    stim_pairs: CollectionStimulusPairs,
    dv: str | VP = "N",
    iv: VPOrList | None = None,
    figsize: Tuple[float, float] = (10, 8),
    alpha: float = 0.7,
    ...
    return _regression_plots(data, dv=dv, ivs=iv_prop, figsize=figsize, alpha=alpha)


def property_ratio_regression(
    stim_pairs: CollectionStimulusPairs,
    dv: str | VP = "N",
    iv: VPOrList | None = None,
    figsize: Tuple[float, float] = (10, 8),
...
def _regression_plots(
    ...
    if dv in ivs:
        raise ValueError(
            f"Dependent variable '{dv.name}' is also"
