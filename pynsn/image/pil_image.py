...
RESAMPLING = int(_Image.Resampling.LANCZOS)
...
def create(
    ...
    return _PILDraw().create_image(nsn_stimulus=nsn_stimulus, antialiasing=antialiasing)


def create_stim_pair(
    ...
    padding: int = 10,
    antialiasing: bool | int = True,
    background_image: _Image.Image | None = None,
) -> _Image.Image:
    ...
    im = (
        create(stim_pair.stim_a, antialiasing),
    ...
    # height, width, center
    bkg_size2 = (
        max_abs_x + _math.ceil(max_width / 2) + padding,
    ...
    if isinstance(background_image, _Image.Image):
        ...
        bkg = _Image.new("RGBA", (bkg_size2[0] * 2, bkg_size2[1] * 2), (0, 0, 0, 0))

    bkg.paste(
        im[0],
        (
            postion_a[0] + bkg_size2[0] - im[0].size[0] // 2,
    ...
    bkg.paste(
        im[1],
        (
            postion_b[0] + bkg_size2[0] - im[1].size[0] // 2,
    ...
    def draw_shape(
        ...
        if isinstance(shape, (_shapes.Ellipse, _shapes.Dot)):
            ...
            if pict.size[0] != shape.size[0] or pict.size[1] != shape.size[1]:
                pict = pict.resize(
                    (int(shape.size[0]), int(shape.size[1])), resample=RESAMPLING
                )
            ...
            _ImageDraw.Draw(image).rectangle(
                tuple(shape.box),  # type: ignore
                fill=shape.colour.value,
            )
        else:
            raise NotImplementedError(f"Shape {type(shape)} NOT YET IMPLEMENTED")

    @staticmethod
    def draw_convex_hull(image, points, convex_hull_colour, opacity, scaling_factor):
        # FIXME opacity is ignored (not yet supported)
        points = _base.cartesian2image_coordinates(points * scaling_factor, image.size)
        last = None
        draw = _ImageDraw.Draw(image)
        for p in _np.append(points, [points[0]], axis=0):
            if last is not None:
                draw.line(
                    _np.append(last, p).tolist(), width=2, fill=convex_hull_colour.value
                )
            last = p
